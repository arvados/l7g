...
def make_yml_and_run(project_uuid, inputdir, ref, chr1, chrM):
    yml_text = '''gvcfdir:
  class: Directory
  location: keep:%s\n''' % inputdir
    yml_text += 'ref: "%s"\n' % ref
...
tagset:
  ...
  location: keep:%s\n''' % (REFFA[ref], AFN[ref], TAGSET)
    ...
    command.extend(["../convert2fastj/gvcf_version/gvcf2fastj-wf.cwl", yml])
...
def main():
    ...
    make_yml_and_run(args.project_uuid, args.inputdir, args.ref, args.chr1, args.chrM)
