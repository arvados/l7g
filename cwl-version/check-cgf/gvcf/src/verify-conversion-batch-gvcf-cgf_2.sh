...
for cgf_fn in $cgf_fns; do
  ...
  gvcf_name="$gvcf_fn"
  gvcf_fn="$gvcf_dir/$gvcf_name/$gvcf_pfx$gvcf_fn$gvcf_sfx"
  ...
  if [[ ! -e "$gvcf_fn.tbi" ]] ; then
    ...
    tdir=`mktemp -d`
    i_gvcf_fn="$tdir/"`basename $gvcf_fn`

    if [[ "$VERBOSE" -eq 1 ]] ; then
      echo "copying gvcf locally, creating temporary file and indexing ($gvcf_fn -> $i_gvcf_fn)" >> $outfile
    fi

    cp $gvcf_fn $i_gvcf_fn
    tabix -f $i_gvcf_fn
  fi

  h0=`tabix $i_gvcf_fn $chrom | \
    pasta -a gvcf-rotini --full-sequence -r <( refstream $ref_fa $chrom ) | \
    pasta -a rotini-alt0 | tr -d '\n' | md5sum | cut -f1 -d' '`
  h1=`tabix $i_gvcf_fn $chrom | \
    pasta -a gvcf-rotini --full-sequence -r <( refstream $ref_fa $chrom ) | \
    pasta -a rotini-alt1 | tr -d '\n' | md5sum | cut -f1 -d' '`

  if [[ ! -e "$gvcf_fn.tbi" ]] ; then
    rm -f $i_gvcf_fn $i_gvcf_fn.tbi
    rmdir $tdir
  fi
